...
- **カメラで巡るナビゲーション**: 惑星上の各建物が About / Products / Skills / Experience / Contact のアクセスポイント。建物の上に浮かぶ稲妻のマーカー（またはカードの「詳しく見る」）を押すとカメラがその建物へ飛び込み、詳細ページが開きます。スクロールすると全建物を通る軌道に沿ってカメラが進み、ドラッグで自由に見回せます（手を離すと、自動回転と同じくらいのゆっくりした速さで軌道へ戻っていきます）。距離の切り替えは、PC ではヘッダーのズームボタン（押すたびに 俯瞰→標準→正面の建物 と巡回）、スマホでは2本指ピンチ（俯瞰↔標準。最接近はマーカーを直接タップ）です。
- **ハイブリッド・レイアウト**: 3Dシーンを背面に固定し、その上に通常のHTMLで組んだ詳細ページをオーバーレイ。3Dの没入感とWebサイトとしての読みやすさを両立させています。
- **カードでエリアを送る**: カードは、いま正面にあるエリアの紹介。PC では画面左に重なって並び、カーソルを乗せて上下にスクロールすると次のカードへ切り替わります。スマホでは画面下のレールになり、横にスワイプすると隣のエリアへ送れます。どちらもカメラが街を回ってそのエリアを正面に連れてきます。
...
- **停止・再生**: 街の動き（自動回転・観覧車・トラム・住人）を押した瞬間に止められます。PC では言語トグルの右のボタン、**スマホでは何もないところを軽くタップ**（動画プレイヤーのような再生／停止のマークが一瞬出ます）。止めている間も自分でドラッグして眺める角度は変えられ、再生すると止めたところから続きます。
- **スマホ専用の組み替え**: `lg` 未満のタッチ画面では、ボタンを減らして指の操作に置き換えています。停止はタップ、ズームはピンチ、セクション一覧は全画面のメニュー、カードは画面下のレール。ノッチとホームインジケータも避けます。PC のレイアウトはそのままです。
...
### 起動方法
PC では画面左下のPCアイコンをクリックしてリンク群を開き、その中の `[>_]` ターミナルアイコンをクリックしてください。スマホではヘッダーのメニューを開くと、下部にリンク群と一緒に並んでいます。
...
├── state/                 # アプリ全体の状態（AppState / Language の Context、URLハッシュ連携、スマホ判定）
...
├── hub/                   # キャンバスの上に浮くホーム画面（PC の枠組みとスマホの全画面メニュー）
│   ├── card/              #   エリアカード（PC のスタックとスマホの下部レール）と、マーカーへ伸びる稲妻の点線

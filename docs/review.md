...
- ~~縦長では地表の灰色ブロックしか見えない~~ → `HANDHELD_VERTICAL_SHARE`（0.20）で水平線がフレームに入るようにした。390×844 で空が125px（高度をスマホ用に引いた 2026-09-12 以降は267px＝画面の31.6%）

**残っているもの**:

- ~~**`NEAR_ORBIT_RADIUS = 50` は依然としてアスペクト比を見ていない。**~~ → **対応済み（2026-09-12）**。`orbitRadiusForZoom(zoom, handheld)` がスマホ用の高度（`HANDHELD_NEAR_ORBIT_RADIUS` = 77.9 / `HANDHELD_ORBIT_RADIUS` = 122.8）を返すようになった。渡すのはアスペクト比ではなく `useHandheld()` の答え——このリポジトリで「スマホか」を答えるのは1箇所だけ、という規則に合わせたため。値は「惑星の縁が画面の3分の1」から逆算。**`HANDHELD_VERTICAL_SHARE` の実測（一周20点）はやり直していない**——引くと押し下げ量は縮む側（215px → 138px）なので、0.20 が隠す数は増えようがない。測り直しが要るのは逆向き（さらに寄せる）の変更のとき

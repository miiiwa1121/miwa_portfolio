...
次の着手順: B-1（プリレンダー本文）→ B-2（OGP）→ B-4（コントラスト）→ B-3（モバイル）→ B-7（空プロジェクト）。B-1 はこのリストで投資対効果が最大。
...
- **ズームコントロールのボタンが画面右端で切れている。** ヘッダーは `justify-between` ＋ `p-7` ＋ `gap-4` で、`text-4xl` のロゴ＋56pxボタン×3が390pxに入らない。
- `hidden lg:flex`（`HubHeader.tsx`）でナビが消えるため、**モバイルの導線はカードのスワイプのみ。**
- **惑星が近すぎる。** `NEAR_ORBIT_RADIUS = 50` は固定値で、ビューポートのアスペクト比を一切見ていない。縦長では水平FOVが狭いので惑星が画面を突き抜け、地表の灰色ブロックしか見えない。デスクトップの `reference/image7.png` に合わせた値がそのまま効いている。

**解決策**:
- ヘッダーを `md` 未満で縮める（ロゴ `text-2xl`、ボタン `w-11 h-11`、`gap-2`）。あるいはボタン群を折り返す。
- `orbitRadiusForZoom()` にアスペクト比を渡し、`aspect < 1` のとき `radius / aspect` 相当まで引く。`frameDistance()` が既に「狭いほうの半画角で合わせる」ロジックを持っているので、同じ考え方を軌道半径にも適用できる。
- モバイル用のナビ（ボトムシートかハンバーガー）を足す。カードのスワイプだけでは「5つのセクションがある」ことが伝わらない。

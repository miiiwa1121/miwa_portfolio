...
- **代わりに読み手が太陽を動かす。** 太陽は止まっていて惑星のほうが回り続けるので、陰影は変化し続ける——カメラ追従が捨てていた半分がここで戻る。
  ...
  - **カメラ側に回したときはフェードで消す**（`sunVisibility()`、視線方向との角度 45°→65° の smoothstep）。ごまかしではなく幾何そのもの——その角度の本物の太陽は視界の外（背後）にあり、惑星が全面的に照らされている状態だけが残るべき。
  - **ドラッグはカメラ自身の軸まわりの回転で、1pxあたりの回転量は `fov / viewportHeight` から導く**（定数にすると特定のウィンドウサイズでしか指に追従しない）。惑星の回転ドラッグとは `sunDraggingRef` で排他。
...
## テスト
- **Vitest**（node環境）。React も three.js も含まない純粋ロジックのみを対象にしているため、DOMもレンダラも不要で全体が1秒未満で走る（440件）。最も重いのは惑星の球殻の生成と水密性の検査。**ループの中で `expect` を数千回呼ばない** — 数万個のブロックを1個ずつ検証すると、惑星を生成するより検証の方が高くつく。集計してから1回だけ検証する。

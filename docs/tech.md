...
### スマホは別の場面として組む（2026-09-12）

**PC の縮小版ではない。** フレームの縦横比が 0.46 になり、下端をカードレールが塞ぐので、構図も操作も前提が変わる。判定は `state/useHandheld.ts` ただ1つ（`(pointer: coarse) and (max-width: 1023px)`）で、`Hub` と `Scene` がそれぞれ自分で呼ぶ。
...
| HOME ボタン | あり | なし（ピンチアウトかロゴ） |
| カメラの縦オフセット | `NEAR_VERTICAL_SHARE` = 0.636 | `HANDHELD_VERTICAL_SHARE` = 0.15 |
...
**カードレールは一方向のまま。** 横ドラッグは「隣へ1つ」という要求を出すだけで、レールの位置は返ってきた `facing` から導出する。ネイティブのスクロールコンテナにしなかったのは、スクロール位置がそれ自体状態になり、カメラがそれを追うと相互更新の輪が閉じるため。

安全領域（ノッチ・ホームインジケータ）は `globals.css` の `.safe-inset` と `layout.tsx` の `viewportFit: "cover"` の組で、**両方が揃わないと機能しない**（cover が無いと `env()` が全部 0 を返す）。

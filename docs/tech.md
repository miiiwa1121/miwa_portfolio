...
**固定光ではこの世界を照らせない。** 5つのエリアは球面上で149.5°に散らばっており、太陽の向きを全球総当たり（2°刻み、16,200通り）しても**「最も暗いエリア」を 0.000 より上げられる向きは存在しない**。旧・固定光 `[18,34,14]` では `contact` が入射角142°（＝夜側）で `skills` の7分の1の明るさしかなく、ツアーはそこを通ってカードがそのエリアを説明していた。定数の調整では解けない種類の問題。

- **`scene/sunLight.ts` の `sunDirection(view, up)` が太陽の向きを決める純関数。** `view` は惑星の中心からカメラへ向かう方向、`up` は `camera.up`。
- **視線軸そのものではなく `SUN_TILT`（0.5 rad ＝ 28.6°）ずらす。** 視線と光軸を一致させると3つ同時に壊れる: カメラを向いた面が全部同じ N·L になって**ボクセルの立体感が消える**、**影が自分の裏に落ちて見えなくなる**（惑星＋290棟の影パスが丸ごと無駄になる）、**ターミネーターが消えて「宇宙の惑星」に見えなくなる**。
- **角度は実測で決めてある。** 正面のエリアは円盤の中心ではなく5アンカーの最近傍なので、ツアー720点で**中央値18.1°・最大40.1°**カメラ方向からずれる。一周での「正面エリアの N·L 最悪値」は 20°→0.565／25°→0.500／28.6°→0.455／40°→0.292／60°→0.000。下限側は逆に、半径50から見た可視域が sub-camera 点から70.4°なので**θ>19.6°でないとターミネーターが画面に出ない**。`SUN_ROLL`（0.6 rad ＝ 34°）は真上からの光を三点照明の key に寄せるためのもの。
- **カメラのフレームで定義しているので極の縮退が無い。** 惑星の北を方位の基準にすると、カメラが極の真上に来たとき基準が定義できない（`ORBIT_MIN_POLAR = 0.15` なので実際に8.6°まで寄れる）。`camera.up` は自由回転で world +Y、セクションで建物の法線、飛行中はその slerp と、**すでに連続であることが保証されている**。`tangentOf(up, view)` で視線軸に直交する成分だけを取るので、`up` がカメラ側に倒れていてもずらし角はきっちり `SUN_TILT`。結果として**太陽はスクリーン座標で常に同じ位置に居る**（カメラリグに付いた key ライト）。
- **影の定数は太陽が動いても再調整が要らない。** `shadow-camera-left/right/top/bottom = ±SCENE_BOUNDING_RADIUS`（34）はシーンの外接半径で**向きに依存せず**、`near/far` は光源の原点からの距離 `SUN_DISTANCE`（41、旧位置 `[18,34,14]` の 40.94 を丸めたもの）から導かれる。**太陽をこの球面上に置いたまま向きだけ変えるかぎり4つとも正しい**——`sunLight.test.ts` がこの内包関係を押さえている。
- **フィル光（`[-20,16,-18]` の星明かり）は固定のまま。** 夜側の縁に冷たいリムが残り、「外に宇宙がある」感じが保たれる。全部の光をカメラに紐づけるとスタジオになる。

### 宇宙の見た目

背景は暗い宇宙紺（`#070a14`、`Hub.tsx` の背景色 div 一箇所で持つ——canvas 自身は透明で、色を塗っていない。理由は次項）。フォグは無い——大気が無い場所には「遠景を距離で溶かす」役が存在しないので、暗さがその役を引き受ける。`ambientLight`/`hemisphereLight` を低く抑えることで、太陽側と反対側がはっきり濃淡を持つ（惑星が自転しているわけではなく、低い環境光の結果としての「昼夜」。太陽のほうがカメラと一緒に回る——前項）。影のカメラ範囲（`shadow-camera-*`）は実際のシーンの大きさ（`SMOOTH_PLANET_RADIUS` ＋最も高い建物）から計算しており、フォグと違って惑星の大きさを変えるたびに retighten が要る。
...
## テスト
- **Vitest**（node環境）。React も three.js も含まない純粋ロジックのみを対象にしているため、DOMもレンダラも不要で全体が1秒未満で走る（434件）。最も重いのは惑星の球殻の生成と水密性の検査。**ループの中で `expect` を数千回呼ばない** — 数万個のブロックを1個ずつ検証すると、惑星を生成するより検証の方が高くつく。集計してから1回だけ検証する。
- 対象: カメラの姿勢計算と飛行の補間（`scene/worldLayout.ts`）、飛行そのものと着地の引き継ぎ（`scene/cameraFlight.ts`）、太陽の向きと影の内包関係（`scene/sunLight.ts`）、球面の幾何・接空間の基底（`scene/planet/planetLayout.ts`）、各セクションの経緯度（`scene/planet/sections.ts`）、ツアー経路（`scene/planet/tour.ts`）、惑星の球殻と地形（`scene/planet/shell.ts`）、雑居ビルの散布（`scene/planet/city.ts`）、プラザと装飾の配置（`scene/planet/decor.ts`）、ジオラマの時計（`scene/sceneClock.ts`）、決定論的ハッシュ（`scene/voxel/rng.ts`）、制作実績のフィルタ（`detail/products/catalog.ts`）、プロジェクトデータの言語解決（`data/project.ts`）、URL の解釈（`state/sectionUrl.ts`）、詳細ページの退場方向（`detail/detailSheet.ts`）、カードのホイール送り（`hub/cardWheel.ts`）、自己紹介の読了判定とスクロール連動の帰還（`hub/about/aboutScroll.ts`）。

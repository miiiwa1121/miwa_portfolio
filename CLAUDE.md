...
npm test             # Vitest 一回実行（270件で約1.1秒）
npm run test:watch
npm run lint
```
...
| `docs/planet-migration.md` | 惑星への移行の**仕様**（まだ実装されていないもの） | 移行の仕様が決まった／変わったとき。段階が完了したら中身を `tech.md` へ移す |
...
**`feature/voxel-planet` ブランチでは、平面の浮遊島を宇宙に浮かぶ球体の土台へ作り替える移行が進行中。** 下の不変条件は現在のコード（平面の島）についてのもので、移行後にどう変わるかは [docs/planet-migration.md](docs/planet-migration.md) に書いてある。**このブランチで 3D を触る前に必ず読むこと。** とくに `framePose` は緯度の高い建物には使えない（世界の +Y 基準なのでカメラが地表の下に潜る）。

`scene/planet/` にはもう純粋ロジックが入っている（まだ画面には出ていない）。ここの不変条件を2つだけ:

- **ツアー経路は緯度経度の空間で補間する。方向ベクトルを slerp / スプラインして正規化してはいけない。** 球面上の補間として「正しい」のはそちらだが、区間ごとに大円をたどることになり、**大円は両端より極側へ膨らむ**（南緯40度・経度差144°で29.8°）。実際にカメラが緯度-71°まで潜った。`camera.up` が +Y なので極付近は `lookAt` が退化する
- **地形のノイズは方向ベクトルの成分の関数にする。緯度経度で書かない。** ±180°の子午線に崖が、極に風車模様が出る（あらゆる経度が1点で出会うのに、ノイズが同じ点に別々の高さを返す）

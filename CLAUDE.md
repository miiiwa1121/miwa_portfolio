...
- **カメラの行き先を決める effect は `3d/Scene.tsx` に1つだけ。** 経路も4つだけ（エリアへ寄る / ホーム視点へ戻る / 自己紹介を読み切った＝何も飛ばさない / スクロールで全景へ）。ここを分散させない
...
- **自己紹介の帰りは飛行ではなくスクロールが握る。** 進捗 0.5〜1 を `glidePose()` に通して毎フレーム構図を組む（`aboutScroll.ts` の `aboutReturnProgress()`）。**着いた後に飛行を足さない** — 読み切った時点で既にホームの構図なので、`onFinish` は `closePage` だけを呼び、行き先の effect はその場合の分岐で何もせず抜ける。末尾の空白は `h-full`（`h-screen` ではない）: スクローラと同じ高さちょうどであることが「進捗1＝本文が画面上端を抜けた」の根拠になっている
